    ...
    if explored_times > 0:
        ...
        if alpha != 0 and caused_delay != 0:
            if eta == 1:
                ...
                u_penalty = ((caused_delay*alpha)**(eta-1)-1) / (1-eta)
        else:
            u_penalty = 0
...
def compute_probability(utility_list):
    ...
    # When the utility values for some routes are too low (lower than -500),
    # applying the softmax function will cause some problems. In this case,
    # all values in the uility list can be first transformed using the same
    # ratio before using the softmax function.
    if all(i > -500 for i in utility_list)==False:
        ratio = min(utility_list) / (-500)
        tmp_list = [i / ratio for i in utility_list]
        prob_list = [np.exp(u) for u in tmp_list]
    else:
...
def update_utility_df(all_routes, delta, alpha, record_df, df, utility_tmp, round_number=1):
    ...
    Parameters:
        all_routes (list): all possible routes for vehicles to take
        delta (float): the exploration factor in the Upper Confidence Bound algorithm
        alpha (float): the penalty factor
        record_df (pandas DataFrame): dataframe recording data in the simulations
        df (pandas DataFrame): dataframe containing utility data
        utility_tmp (pandas DataFrame): output from the function "utility_summary"
