        ...
        for i in road_list:
            ...
            if vehicle.current_road_index + 1 < len(vehicle.path):
                ...
                if (vehicle.x >= (road.length - road.stop_distance)) and (vehicle.x < road.length):
                    if (len(self.roads[next_road_index].vehicles) != 0) and (self.roads[next_road_index].vehicles[0].x < (vehicle.s0 + vehicle.l)):
                        # vehicle.slow(0.2*vehicle._v_max)
                        vehicle.stop()

                    if vehicle.v <= 0:
                        vehicle.unstop()
                    elif wait_time > 0 and (road.length - vehicle.x)/(vehicle._v_max*wait_time)>=1:
                        # vehicle.unslow()
                        vehicle.unstop()
                    elif wait_time > 0 and (road.length - vehicle.x)/(vehicle.v*wait_time)<1:
                        # vehicle.slow(0.2*vehicle._v_max)
                        vehicle.stop()
                    if wait_time<=0:
                        self.roads[next_road_index].wait_time = self.roads[next_road_index]._wait_time
                        # print(f"Reset waiting time: {self.roads[next_road_index].wait_time}")
                        # vehicle.unslow()
                        vehicle.unstop()
                # When vehicle is out of road bounds
                elif vehicle.x >= road.length:

                    cond_1 = (len(self.roads[next_road_index].vehicles) == 0) or (self.roads[next_road_index].vehicles[0].x >= (vehicle.s0 + vehicle.l))
                    cond_2 = wait_time <= 0
                    # print(vehicle.path[vehicle.current_road_index], cond_1 and cond_2)
