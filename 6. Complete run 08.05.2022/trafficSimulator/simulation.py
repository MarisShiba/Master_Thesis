...
#                 print(len(self.roads), start, end, slow_factor, stop_distance, wait_time)
                    ...
                    if (vehicle.x >= (road.length - road.stop_distance)) and (vehicle.x < road.length):
                        if j == 0:
                            ...
                            if wait_time <= 0:
                                ...
                                # print(f"Stop now: Vehicle label {j}, wait_time left is {j*self.roads[next_road_index]._wait_time+wait_time},\
                                # speed is {vehicle.v}, distance left is {road.length - vehicle.x}")
...
#                             vehicle.v_max = self.roads[next_road_index].slow_factor*vehicle._v_max
                        ...
                        if j*self.roads[next_road_index]._wait_time+wait_time > 0 and j != 0:
                            # print(f"Vehicle label {j}, wait_time left is {j*self.roads[next_road_index]._wait_time+wait_time},\
                            # speed is {vehicle.v}, distance left is {road.length - vehicle.x}, a is {vehicle.a}")
                            ...
                            vehicle.stopped = False
                    # elif vehicle.x < (road.length - road.stop_distance):
                    #     vehicle.stopped = False
