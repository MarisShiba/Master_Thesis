...
class Vehicle:
    ...
    def set_default_config(self):
        ...
        self.be_front = False # Whether the vehicle is the leading one on the current road
        self.total_time = 0
    ...
    def update(self, lead, dt, records):
        ...
        if self.v == 0: # When the vehicle is stopping
            if lead_v[self.current_road_index] != -999: # The delay time record
                                                        # is only helpful when the
                                                        # vehicle has a leading vehicle
                stop_time[self.current_road_index] += dt
            if self.be_front == True: # When the vehicle is waiting to enter the next
                                      # road, its waiting time is recorded so that a
                                      # priority can be assigned to vehicles that try
                                      # to enter the same road but are originally from
                                      # different roads
