...
class Road:
    def __init__(self, start, end, slow_factor=1, stop_distance=50, wait_time=2):
        self.start = start
        self.end = end
        ...
        self.vehicles = deque()
        self.driven = False # To differentiate if any vehicles have entered the road
    ...
    def update(self, dt, records, front_lead=None):
        ...
        if n > 0:
            ...
            # Adjust the vehicle's desired speed to the current road's speed limit
            self.vehicles[0].v_max = self.slow_factor * self.vehicles[0]._v_max
            ...
            for i in range(1, n):
                ...
                # Adjust the vehicle's desired speed to the current road's speed limit
                self.vehicles[i].v_max = self.slow_factor * self.vehicles[i]._v_max
        try:
            return new_records
        except:
            return records
