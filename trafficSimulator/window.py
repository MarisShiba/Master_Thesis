...
class Window:
    ...
    def loop(self, loop=None):
        ...
        while running:
            ...
            # When the condition for stopping is met (no vehicles have passed all
            # roads), then stop the simulation as well
            if len(self.sim.stop_condition)==0:
                running = False
    ...
    def draw_status(self):
        text_fps = self.text_font.render(f'Time={round(self.sim.t, 1)}', False, (0, 0, 0))
        text_round = self.text_font.render(f'Round number={self.sim.round_number}', False, (0, 0, 0))
        try:
            text_vec = self.text_font.render(f'Vehicle number={self.sim.generators[0].vehicle_cnt}', False, (0, 0, 0))

        except:
            text_vec = self.text_font.render('Vehicle number=0', False, (0, 0, 0))

        self.screen.blit(text_fps, (6, 0))
        self.screen.blit(text_vec, (6, 40))
        self.screen.blit(text_round, (6, 80))
